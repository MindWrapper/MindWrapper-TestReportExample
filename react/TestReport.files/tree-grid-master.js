(function () {
        ...
        .run([
            ...
            function ($templateCache) {
                $templateCache.put('template/treeGrid/treeGrid.html',
                    ...
                    "   </thead>\n" +
                    "   <tbody>\n" +
                    "     <tr ng-repeat-start=\"row in tree_rows | searchFor:$parent.filters:$parent.additionalFilter:$parent.search:expandingProperty:colDefinitions track by row.branch.uid\"\n" +
                    ...
                    "<table table style=\"width: 100%; table-layout: fixed;\">" +
                        "<tr>" +
                            "<td style=\"width: 30px;\">" +
                                "<a ng-click=\"user_clicks_branch(row.branch, $event)\"><i ng-class=\"row.tree_icon\" ng-click=\"row.branch.expanded = !row.branch.expanded\" class=\"indented tree-icon tree-icon-button\"></i></a>" +
                            "</td>" +
                            "<td style=\"width: 30px;\">" +
                                "<a><i ng-class=\"row.branch.detailed ? 'glyphicon glyphicon-triangle-top indented tree-icon tree-icon-button' : 'glyphicon glyphicon-triangle-bottom indented tree-icon tree-icon-button'\" title=\"{{::row.branch.detailed ? 'Show less' : 'Show more'}}\"></i></a>" +
                            "</td>" +
                            "<td>" +
    ...
    angular
            ...
            function ($timeout,
                ...
                return {
                    ...
                    link: function (scope, element, attrs) {
                        ...
                        scope.$watchCollection('treeData', on_treeData_change);
        ...
        .filter('searchFor', function () {
            return function (arr, filterString, additionalFilter, search, expandingProperty, colDefinitions, expand) {
                var filtered = [];
                ...
                if(!filterString || !filterString.length === 0) {
                    ...
                    for (var i = 0; i < arr.length; i++) {
                        ...
                        if (include(item, filterString, additionalFilter, search, expandingProperty, colDefinitions)) {
            ...
            function include(item, filterString, additionalFilter, search, expandingProperty, colDefinitions) {
                var includeItem = false;
                var filterApplied = false;
                //first check the expandingProperty
                if (expandingProperty.filterable) {
                    filterApplied = true;
                    if (checkItem(item, filterString, additionalFilter, search, expandingProperty)) {
                        includeItem = true;
                    }
                }
            ...
            function checkItem(item, filterString, additionalFilter, search, col) {
                ...
                return true && checkSearchString(item, search);
            }

            function checkSearchString(item, search) {
                if(!search || search.length < 3) {
                    return true;
                }

                var propertiesToSearch = ['name', 'message', 'fixture', 'arguments'];
                for(var i=0; i<propertiesToSearch.length; i++) {
                    if(item.branch[propertiesToSearch[i]] !== undefined && item.branch[propertiesToSearch[i]].toLowerCase().indexOf(search.toLowerCase()) !== -1)
                        return true;
                }
